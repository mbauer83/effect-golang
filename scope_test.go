...
type scopedProgram = effect.Effect[effect.Unit, string, string]

// acquire builds a resource whose acquisition and release are both recorded, so
// a test can assert ordering and exactly-once release.
func acquire(tracker *effecttest.Tracker, scope effect.Scope, name string) scopedProgram {
	...
	return scope.AcquireRelease(
		operations.From(func(context.Context, effect.Unit) effect.Exit[string, string] {
			...
			return effect.ExitSuccess[string](name)
		}),
		func(resource string) effect.Effect[effect.Unit, effect.Never, effect.Unit] {
			return effect.Release[effect.Unit](func(context.Context) error {
				tracker.Record("release " + resource)
				return nil
			})
		},
	)
...
func TestScopeReleasesResourcesInReverseAcquisitionOrder(t *testing.T) {
	...
	program := effect.Scoped(func(scope effect.Scope) scopedProgram {
		return acquire(tracker, scope, "database").
			FlatMap(func(string) scopedProgram {
				return acquire(tracker, scope, "transaction")
			}).
			FlatMap(func(string) scopedProgram {
				return acquire(tracker, scope, "statement")
			})
	})
...
func TestScopeReleasesExactlyOnceForEveryOutcome(t *testing.T) {
	...
	outcomes := map[string]scopedProgram{
		...
		"defect":      operations.From(panicking),
		"interrupted": operations.From(selfCanceling),
	}

	for name, body := range outcomes {
		tracker := &effecttest.Tracker{}
		program := effect.Scoped(func(scope effect.Scope) scopedProgram {
			return acquire(tracker, scope, "handle").AndThen(body)
		})
...
func panicking(context.Context, effect.Unit) effect.Exit[string, string] {
	panic("body exploded")
}

func selfCanceling(context.Context, effect.Unit) effect.Exit[string, string] {
	return effect.ExitCause[string, string](effect.InterruptCause[string](context.Canceled))
}
...
func TestNestedScopesReleaseInnerLifetimeFirst(t *testing.T) {
	...
	program := effect.Scoped(func(outer effect.Scope) scopedProgram {
		return acquire(tracker, outer, "outer").FlatMap(func(string) scopedProgram {
			return effect.Scoped(func(inner effect.Scope) scopedProgram {
				return acquire(tracker, inner, "inner")
			})
		})
	})
...
func TestAcquisitionAfterClosureReleasesImmediatelyAndReportsClosedScope(t *testing.T) {
	...
	exit := effect.Run(context.Background(), effect.Unit{}, acquire(tracker, escaped, "late"))
...
func TestScopeReleasesUnderCallerCancellation(t *testing.T) {
	...
	program := effect.Scoped(func(scope effect.Scope) scopedProgram {
		return acquire(tracker, scope, "handle").FlatMap(func(string) scopedProgram {
			cancel(stop)
