...
func runGenBody[R, E, A any](do *Do[R, E], body func(*Do[R, E]) A, result chan<- Exit[E, A]) {
	returned := false
	var exit Exit[E, A]
	defer func() {
		panicValue := recover()
		...
		case returned:
		case panicValue != nil:
			exit = ExitCause[E, A](DieCause[E](Defect{Value: panicValue, Stack: string(debug.Stack())}))
		case do.failed:
