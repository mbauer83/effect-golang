...
type Finalizer func(context.Context, Exit) Cause
...
func (scope *Scope) AddFinalizer(ctx context.Context, finalizer Finalizer) (bool, Cause) {
	scope.mutex.Lock()
...
func (scope *Scope) Close(ctx context.Context, exit Exit, reason error) Cause {
...
func (scope *Scope) beginClosing(exit Exit) ([]Finalizer, bool) {
	scope.mutex.Lock()
	defer scope.mutex.Unlock()
	if scope.status != scopeOpen {
		return nil, false
...
func runFinalizer(ctx context.Context, finalizer Finalizer, exit Exit) (cause Cause) {
	defer func() {
		if recovered := recover(); recovered != nil {
			cause = DieCause(CapturedDefect(recovered))
		}
	}()
