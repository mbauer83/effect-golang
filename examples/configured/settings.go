...
// StoreDescription is the storage component's own description of its settings.
//
...
func MailerDescription() config.Config[Mailer] {
	return config.Nested("mail", config.Struct(
		config.Setting(
			config.NonEmptyText("sender").WithDescription("the address mail is sent from"),
			func(mailer *Mailer, sender string) { mailer.Sender = sender }),
		config.Setting(
			config.Many("relays", ",", config.NonEmptyText("")).
				WithDescription("the relays to try, in order"),
			func(mailer *Mailer, relays []string) { mailer.Relays = relays }),
		config.Setting(
			config.Int("retries").
				WithDefault(3).
				Validate("at most ten retries", func(retries int) bool {
...
// ReportSchedule is a plugin's settings, and it is deliberately small: what it
...
// ReportScheduleDescription reads a schedule, and folds absence into the value rather
// than into an option nobody can forget to check.
func ReportScheduleDescription() config.Config[ReportSchedule] {
	return config.Optional(
		config.Duration("every").WithDescription("how often to report, if at all"),
		func(every time.Duration) ReportSchedule {
			return ReportSchedule{Every: every, Enabled: true}
		},
		func() ReportSchedule {
