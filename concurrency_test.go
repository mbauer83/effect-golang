...
// barrier releases only once every participant has arrived, which is what makes
// a concurrency test honest: a sequential implementation would deadlock instead
// of quietly passing.
type barrier struct {
...
func meeting(meetingPoint *barrier, tracker *effecttest.Tracker, name string, result effect.Exit[string, string]) forkedProgram {
...
func TestZipParAwaitsTheCanceledSiblingsCleanup(t *testing.T) {
	...
	sibling := effect.Scoped(func(scope effect.Scope) forkedProgram {
		return acquire(tracker, scope, "sibling-handle").AndThen(work.program())
	})
...
func TestRaceFirstLetsTheFirstCompletionWinEvenWhenItFailed(t *testing.T) {
	...
	loser := effect.Scoped(func(scope effect.Scope) forkedProgram {
		return acquire(tracker, scope, "loser-handle").AndThen(work.program())
	})
...
func TestParallelCompositionPropagatesCallerCancellation(t *testing.T) {
	...
	left := newBlocker(tracker)
	right := newBlocker(tracker)
	...
	exit := effect.Run(ctx, effect.Unit{}, effect.ZipPar(left.program(), right.program()))
	cause, failed := exit.Cause()
